...
export function getStickyModel(messages) {
...
export function setStickyModel(messages, modelDbId) {
    const key = getSessionKey(messages);
    if (!key)
        return;
...
export function isRetryableError(err) {
    ...
    return msg.includes('429') || msg.includes('rate limit') || msg.includes('503')
        || msg.includes('500') || msg.includes('timeout') || msg.includes('api error 400')
        || msg.includes('404') || msg.includes('402') || msg.includes('payment required');
}
...
proxyRouter.post('/embeddings', async (req, res) => {
    ...
    try {
        ...
        const vectors = await Promise.all(inputs.map(async (text) => {
...
proxyRouter.post('/chat/completions', async (req, res) => {
    ...
    const detailBase = {
        traceId,
        clientModel: requestedModel ?? AUTO_MODEL_ID,
        stream: !!stream,
        hasTools: !!(tools?.length),
        messageCount: messages.length,
        promptPreview: buildPromptPreview(messages),
        clientIp: req.ip || req.socket?.remoteAddress || null,
        userAgent: req.headers['user-agent'] ?? null,
    };
    const logDetailAttempt = (attempt, route, status, opts = {}) => {
        logRequestDetail({
            ...detailBase,
            attempt,
            routedPlatform: route?.platform ?? null,
            routedModel: route?.modelId ?? null,
            displayName: route?.displayName ?? null,
            keyId: route?.keyId ?? null,
            status,
            inputTokens: opts.inputTokens ?? estimatedInputTokens,
            outputTokens: opts.outputTokens ?? 0,
            latencyMs: opts.latencyMs ?? (Date.now() - start),
            ttfbMs: opts.ttfbMs ?? null,
            error: opts.error ?? null,
            responsePreview: opts.responsePreview ?? null,
            finalSuccess: !!opts.finalSuccess,
        });
    };
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        ...
        try {
            if (stream) {
                ...
                try {
                    ...
                    for await (const chunk of gen) {
                        ...
                        if (text)
                            streamText += text;
                        const toolDelta = chunk?.choices?.[0]?.delta?.tool_calls;
                        if (toolDelta?.length) {
                    ...
                    logDetailAttempt(attempt, route, 'success', {
                        ...
                        ttfbMs,
                        responsePreview: truncateText(streamText) || '(пустой ответ в stream)',
                        finalSuccess: true,
                    });
                    recordModelOutcome(route.modelDbId, true);
                    return;
                }
                catch (streamErr) {
                    if (streamStarted) {
                        ...
                        const streamErrMsg = sanitizeProviderErrorMessage(streamErr.message);
                        logRequest(route.platform, route.modelId, route.keyId, 'error', estimatedInputTokens, totalOutputTokens, Date.now() - start, streamErrMsg);
                        logDetailAttempt(attempt, route, 'error', {
                            inputTokens: estimatedInputTokens,
                            outputTokens: totalOutputTokens,
                            latencyMs: Date.now() - start,
                            error: streamErrMsg,
                            responsePreview: truncateText(streamText),
                        });
                        return;
                    }
            ...
            recordModelOutcome(route.modelDbId, false);
            if (isRetryableError(err)) {
                // Put this model+key on cooldown and try the next one
                const skipId = `${route.platform}:${route.modelId}:${route.keyId}`;
                skipKeys.add(skipId);
                setCooldown(route.platform, route.modelId, route.keyId, getCooldownDurationForLimit(route.platform, route.modelId, route.keyId, {
                    rpd: route.rpdLimit,
                    tpd: route.tpdLimit,
                }));
    ...
    const exhaustedMsg = sanitizeProviderErrorMessage(lastError?.message);
    logRequestDetail({
        ...detailBase,
        attempt: MAX_RETRIES,
        status: 'error',
        error: `All models failed after ${MAX_RETRIES} attempts. Last: ${exhaustedMsg}`,
        latencyMs: Date.now() - start,
        inputTokens: estimatedInputTokens,
    });
    res.status(503).json({
        error: {
            message: `All models failed after ${MAX_RETRIES} attempts. Last: ${exhaustedMsg}`,
