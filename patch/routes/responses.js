...
import { recordModelOutcome } from '../lib/custom-router.js';
import { newTraceId, logRequestDetail, buildPromptPreview, buildResponsePreview, } from '../lib/request-detail-log.js';
...
responsesRouter.post('/responses', async (req, res) => {
    ...
    let lastError = null;
    const clientModel = reqData.model ?? 'auto';
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        let route;
        try {
            route = routeRequest(estimatedTotal, skipKeys.size > 0 ? skipKeys : undefined, preferredModel, false, routingAudit);
        }
        catch (err) {
        ...
        try {
            if (stream) {
                ...
                logDetailAttempt(attempt + 1, route, 'success', {
                    inputTokens: estimatedInputTokens,
                    outputTokens: totalOutputTokens,
                    responsePreview: buildResponsePreview(msgText),
                    finalSuccess: true,
                });
            ...
            logDetailAttempt(attempt + 1, route, 'error', { error: err.message, latencyMs: latency });
